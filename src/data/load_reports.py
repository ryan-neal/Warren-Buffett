...
import textract
import numpy as np
from src.data.market_returns import clean_buffett, get_market_returns, clean_sp, get_difference
...
class Report():
    def __init__(self):
        # Required variables
        self.FIELDS = ['year', 'text', 'brk-returns', 's&p-returns', 'vs-market']
    ...
    def data_from_file(self, file_path):
        ...
        data = {
            ...
            'vs-market': get_difference(x).loc[str("".join(re.findall(r'\d+', os.path.basename(file_path))))]
        }
        return data
